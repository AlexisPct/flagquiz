...
import "./QuizAnswersAutocompleteInput.css";
...
export const QuizAnswersAutocompleteInput: React.FC<
  QuizAnswersAutocompleteInputProps
> = ({
  questionId,
  autocompleteList,
  feedback,
    ...
    <div className="quiz-input-wrapper">
      <div className="quiz-relative-container">
        <input
          ...
          onFocus={() =>
            inputValue.trim().length >= 2 && setShowSuggestions(true)
          }
        ...
        {showSuggestions && suggestions.length > 0 && !feedback && (
          <ul className="quiz-suggestions-list">
            {suggestions.map((answer, idx) => (
              <li
                key={idx}
                onClick={() => selectSuggestion(answer)}
                className="quiz-suggestion-item"
              >
                {answer}
              </li>
            ))}
