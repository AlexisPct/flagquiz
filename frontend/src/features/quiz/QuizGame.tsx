...
export const QuizGame: React.FC = () => {
  ...
  const [currentQuestion, setCurrentQuestion] = useState<ServerQuestion | null>(
    null,
  );
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch((err) =>
        console.error(
          "Erreur de chargement des données d'autocomplétion :",
          err,
        ),
      );
  }, []);

  const currentAutocompleteList =
    config.type === "capital" ? capitalsList : countriesList;
